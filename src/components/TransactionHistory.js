  ...
  const handleLogout = async () => {
    ...
    const getData = async () => {
      ...
      try {
        ...
        const balance_query = query(collection(db, "users"), where("email", "==", email));
    ...
    <div>
      ...
      <nav className="navbar navbar-expand-lg navbar-dark bg-dark shadow">
        ...
        <Link className="navbar-brand d-flex align-items-center" to="/home">
          ...
          MS Bank</Link>
          <div className="text-light">Current Balance:
          <NumberFormat 
          ...
          <div className="collapse navbar-collapse justify-content-end" id="navbarNavDropdown">
            <ul className="navbar-nav">
              ...
              {hour>=12 ? hour>=16? <span className="fw-nomal">Good Evening, </span>: <span className="fw-normal">Good Afternoon, </span>: <span className="fw-normal">Good Morning, </span>}
                ...
                <button className="fw-bold active btn btn-light" id="user_profile_logout" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                  {firstName}
                </button>
                ...
                <li><Link className="text-decoration-none" to="/Home"><button className="dropdown-item bg-dark text-light">Home</button></Link></li>
                  <li><Link className="text-decoration-none" to="/Profile"><button className="dropdown-item bg-dark text-light">Profile</button></Link></li>
                ...
                <tbody>
                  {transaction.map((doc,index) => {
                    return ( <tr key ={doc.id}>
                      ...
                      <td>{doc.txn_id}</td>
                      <td>{new Date(doc.dateTime).toLocaleDateString('en-IN',options) + " " + new Date(doc.dateTime).toLocaleTimeString('en-US')}</td>
                      <td>{doc.desc}</td>
